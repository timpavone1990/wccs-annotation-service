...
function processPropertiesAndReferences(object, objectPath, annotations) {
    ...
    composeAnnotations(object, `${objectPath}${separator}contents`, "contents", annotations);
...
function processFeature(feature, featureType, featurePath, annotations) {
    if (featureType === "references") {
    ...
    } else if (featureType === "contents") {
        if (feature.hasOwnProperty("contents") && Object.keys(feature.contents).length > 0
        ...
        if (feature.content) {
            const annotation = createAnnotation(feature, featurePath, "content");
            annotations.push(annotation);
        }
    }
}
...
function createAnnotation(feature, featurePath, featureKind) {
    ...
    return {
        ...
        "text": `${featureName}: ${feature.class}`,
        "ranges": [{
